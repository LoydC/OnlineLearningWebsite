...
(function() {
	var a = (function() {
		var f = {
				...
				basePath : (function() {
					...
					if (i.indexOf('://') == -1)
						if (i.indexOf('/') === 0)
							i = location.href.match(/^.*?:\/\/[^\/]*/)[0] + i;else
	...
	function d(f) {
		var i = this;
		i.id = f.name;var g = f.element.getDocument().getWindow().$,
			h = a.oC.getWindow().$;
	...
	function e(f, g, h) {
		g.on('appReady', function(i) {
			...
			f.config = g.config;g.aG['foldertree.foldertree'].on('afterAddFolder', function(j) {
				j.removeListener();
				if (h) h(f);
			}, f);
		}, f, null, 999);
	...
	;d.prototype = {
		...
		addFileContextMenuOption : function(f, g, h) {
			...
			if (!f.group)
				f.group = 'file1';
			i.gp(j, f);i.aG['filesview.filesview'].on('beforeContextMenu', function m(k) {
				if (h) {
		...
		disableFileContextMenuOption : function(f, g) {
			...
			h.aG['filesview.filesview'].on('beforeContextMenu', j);return function() {
		...
		addFolderContextMenuOption : function(f, g, h) {
			...
			if (!f.group)
				f.group = 'folder1';
			i.gp(j, f);i.aG['foldertree.foldertree'].on('beforeContextMenu', function m(k) {
				if (h) {
					var l = h(this.app.aV);
					if (l)
						k.data.bj[j] = l == -1 ? a.aY : a.aS;
		...
		disableFolderContextMenuOption : function(f, g) {
			...
			h.aG['foldertree.foldertree'].on('beforeContextMenu', j);return function() {
		...
		disableFolderDropMenuOption : function(f, g) {
			...
			h.aG['foldertree.foldertree'].on('beforeDropMenu', j);return function() {
		...
		replaceUploadForm : function(f, g, h, i) {
			...
			if (!i)
				i = 10;
			if (i >= (j.dC || 20)) return;
			j.dC = i;j.aG['formpanel.formpanel'].on('beforeUploadFileForm', function(k) {
				...
				try {
					...
					this.oW('failedUploadFileForm', k.data);this.oW('afterUploadFileForm', k.data);
					throw a.ba(n);
				}
			});return {
				hide : function() {
		...
		resizeFormPanel : function(f) {
			...
			if (typeof f == 'undefined') {
				g.document.getById('panel_view').setStyle('height', '');g.document.getById('panel_widget').setStyle('height', '');
			} else {
				var h = Math.min(f, Math.max(90, g.document.getById('main_container').$.offsetHeight - 300));
				g.document.getById('panel_view').setStyle('height', h + 'px');g.document.getById('panel_widget').setStyle('height', h - 3 + 'px');
			}
			g.layout.ea(true);
		},
		...
		function f(g) {
			...
			CKFinder._.instanceConfig[h] = g;return h;
		}
		;CKFinder.prototype = {
			create : function(g) {
			...
			popup : function(g, h) {
				...
				o.$.write(n);o.$.close();try {
					var p = navigator.userAgent.toLowerCase();
					if (p.indexOf(' chrome/') == -1) {
						m.moveTo(j, i);m.resizeTo(g, h);
					}
					m.focus();return q.appendTo(o.bH());
				} catch (r) {
					...
					if (n.mi(i) < 0) {
						...
						var p = this,
							q = function(s, t, u, v) {
								...
								i.call(j, w);return w.data;
							};
						q.fn = i;
						q.nT = l;
						for (var r = o.length - 1; r >= 0; r--) {
							if (o[r].nT <= l) {
								o.splice(r + 1, 0, q);return;
							}
						}
						o.unshift(q);
					}
	...
	if (!a.application) {
		...
		a.application = function(f, g, h, i) {
			...
			j.ff = h || a.kZ;a.event.call(j);j.iI(i);
		};
	...
	if (a.status == 'unloaded') (function() {
			...
			a.dO = function() {
				...
				i.src = a.basePath + 'ckfinder.js';document.getElementsByTagName('head')[0].appendChild(i);
			};
			a.mS = 0;
			a.uQ = 'ckfinder';
			a.uM = true;
	...
	a.ajax = (function() {
		var i = function() {
			...
			m = function(n, o, p, q) {
				...
				if (q) {
					s.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');s.send(q);
				} else s.send(null);
				return r ? '' : p(s);
			};
		return {
	...
	CKFinder.ajax = a.ajax;(function() {
		...
		a.tools = {
			...
			htmlEncode : function(j) {
				...
				this.htmlEncode = n;return this.htmlEncode(j);
			},
			...
			createClass : function(j) {
				...
				if (l) {
					...
					k.prototype.base = function() {
						this.base = l.prototype.base;l.apply(this, arguments);
			...
			mH : function(j) {
				if (g) {
					...
					j.$.addEventListener('focus', function() {
						j.addClass('focus_inside');
					}, true);j.$.addEventListener('blur', function() {
						j.removeClass('focus_inside');
					}, true);
				}
			},
	...
	h.event.prototype = {
		...
		preventDefault : function(j) {
			var k = this.$;
			if (k.preventDefault) k.preventDefault();else
				k.returnValue = false;
			if (j) this.stopPropagation();
		},
		stopPropagation : function() {
			var j = this.$;
			if (j.stopPropagation) j.stopPropagation();else
				j.cancelBubble = true;
		},
		bK : function() {
	...
	h.window.prototype = new h.dE();i.extend(h.window.prototype, {
		focus : function() {
	...
	var j = h.document;
	j.prototype = new h.dE();i.extend(j.prototype, {
		...
		getHead : function() {
			...
			k = new h.element(k);return (this.getHead = function() {
				return k;
			})();
		},
		bH : function() {
	...
	h.bi = function(k) {
		if (k) {
			switch (k.nodeType) {
			case a.cv:
				return new h.element(k);case a.fl:
				return new h.text(k);
			}
			h.dE.call(this, k);
		}
	...
	a.gX = 4;
	a.mo = 8;
	a.lF = 16;i.extend(h.bi.prototype, {
		appendTo : function(k, l) {
			k.append(this, l);return k;
		},
		clone : function(k, l) {
			...
			if (!l) {
				var n = function(o) {
					...
					o.removeAttribute('id', false);o.removeAttribute('_ckf_expando', false);
					var p = o.childNodes;
		...
		kB : function(k) {
			k.$.parentNode.insertBefore(this.$, k.$.nextSibling);return k;
		},
		insertBefore : function(k) {
			k.$.parentNode.insertBefore(this.$, k.$);return k;
		},
		vP : function(k) {
			this.$.parentNode.insertBefore(k.$, this.$);return k;
		},
		lU : function(k) {
			var l = [],
			...
			while (n.getFirst && (k = n.getFirst())) {
				if (k.type == a.fl) {
					...
					if (!l) {
						...
						k.split(m - l.length);n.$.removeChild(n.$.firstChild);
					}
				}
				break;
			}
			...
			while (n.dB && (k = n.dB())) {
				if (k.type == a.fl) {
					...
					if (!l) {
						...
						k.split(l.length);n.$.lastChild.parentNode.removeChild(n.$.lastChild);
					}
				}
				break;
			}
	...
	k.kE = function(l, m) {
		var n = new k('div', m);
		n.setHtml(l);return n.getFirst().remove();
	};
	...
	k.qZ = function(l, m, n) {
		...
		append : function(l, m) {
			...
			a.log('[DOM] DOM flush into ' + n.getName());return l;
		},
		...
		appendText : function(l) {
			if (this.$.text != undefined)
				this.$.text += l;else this.append(new h.text(l));
		},
		pd : function() {
			var m = this;
			var l = m.dB();
		...
		tV : function(l) {
			...
			m.setStartAfter(o);m.setEndAfter(l);
			var n = m.extractContents();
			m.insertNode(o.remove());n.kA(o);
		},
		...
		fH : function() {
			...
			l.appendChild(m.$.cloneNode(true));return l.innerHTML;
		},
		setHtml : function(l) {
			a.log('[DOM] DOM flush into ' + this.getName());return this.$.innerHTML = l;
		},
		setText : function(l) {
			k.prototype.setText = this.$.innerText != undefined ? function(m) {
				a.log('[DOM] Text flush');return this.$.innerText = m;
			} : function(m) {
				a.log('[DOM] Text flush');return this.$.textContent = m;
		...
		setAttribute : (function() {
			var l = function(m, n) {
				this.$.setAttribute(m, n);return this;
			};
			if (g && (f.ie7Compat || f.ie6Compat)) return function(m, n) {
					...
					if (m == 'class')
						...
						o.$.checked = n;else l.apply(o, arguments);
					return o;
				};else return l;
		})(),
		...
		setStyle : function(l, m) {
			this.$.style[i.cssStyleToDomStyle(l)] = m;return this;
		},
		...
		setOpacity : function(l) {
			if (g && f.version < 9) {
				l = Math.round(l * 100);this.setStyle('filter', l >= 100 ? '' : 'progid:DXImageTransform.Microsoft.Alpha(opacity=' + l + ')');
		...
		bR : function(l) {
			...
			switch (l) {
			case a.eV:
				m.addClass('cke_on');m.removeClass('cke_off');m.removeClass('cke_disabled');
				break;case a.aY:
				m.addClass('cke_disabled');m.removeClass('cke_off');m.removeClass('cke_on');
				break;default:
				m.addClass('cke_off');m.removeClass('cke_on');m.removeClass('cke_disabled');
				break;
			}
		},
		...
		renameNode : function(l) {
			...
			var m = o.getDocument(),
				n = new k(l, m);
			o.qw(n);o.jg(n);o.$.parentNode.replaceChild(n.$, o.$);
	...
	a.fs.prototype = {
		...
		a.log = function() {
			...
			if (typeof window.console == 'object')
				if (!console.log.apply) console.log(m);else console.log.apply(console, arguments);
		};
		a.ba = function(m) {
		...
		return {
			load : function(o, p, q, r, s) {
				...
				var u = o.length,
					...
					x = function(D) {
						if (p)
							if (t) p.call(q, D);else p.call(q, v, w);
					};
				if (u === 0) {
					x(true);return;
				}
							...
							if (g)
								F.$.onreadystatechange = function() {
									if (F.$.readyState == 'loaded' || F.$.readyState == 'complete') {
										F.$.onreadystatechange = null;a.log('[LOADED] ' + D);z(D, true);
									}
								};else {
								F.$.onload = function() {
									setTimeout(function() {
										a.log('[LOADED] ' + D);z(D, true);
									}, 0);
								};
								F.$.onerror = function() {
									z(D, false);
			...
			uq : function(o) {
				var p = new k('script');
				p.setAttribute('type', 'text/javascript');p.appendText(o);p.appendTo(a.document.getHead());
			}
		};
	})();
		...
		var n = {},
			o = function(p, q) {
				var r = function() {
					...
					s = new k('img');
				s.on('load', r);s.on('error', r);s.setAttribute('src', p);
			};
		a.rw = {
			load : function(p, q) {
				var r = p.length,
		...
		var n = {},
			...
			q = function(r, s, t, u) {
				...
				if (y) u && u();else {
					...
					if (!B) {
						w(t.js);r.scriptLoader.load(t.js, function() {
							B = 1;C();
						});
					}
					C();
		...
		return {
			...
			load : function(r, s, t) {
				...
				if (n[u]) {
					...
					p[u] = v;a.scriptLoader.load(v + 'skin.js', function() {
						q(r, u, s, t);
					});
				}
			}
	...
	n.prototype = {
		...
		var o = 0,
				...
				if (A.fn) {
					...
					if (y.config.customConfig == z || !r(y)) y.cr('customConfigLoaded');
						...
						if (CKFinder.customConfig)
							A.fn = CKFinder.customConfig;else
							A.fn = function() {};
						r(y);
					});
				return true;
			...
			t = function(y) {
				...
				y.qn = y.ox();y.on('uiReady', function() {
					y.document.getWindow().on('lW', function() {
						i.setCookie('CKFinder_UTime', Math.round(new Date().getTime() / 1000), true);i.setCookie('CKFinder_UId', encodeURIComponent(y.id ? y.id : location.href), true);
					});
				});y.cr('configLoaded');u(y);
			},
			u = function(y) {
				a.lang.load(y.config.language, y.config.defaultLanguage, function(z, A) {
					...
					y.lB = (function() {
						...
						C = "'" + C + "'";
						C = C.replace(/('',)|,''$/g, '');return new Function('day', 'month', 'year', 'hour', 'minute', 'return [' + C + "].join('');");
					})();
					...
					a.scriptLoader.load(H, function() {
						...
						for (var O = 0; O < N.length; O++)
							for (var P = 0; P < F.length; P++) {
								...
								if (Q[N[O]]) {
									a.log('[PLUGIN] ' + Q.name + '.' + N[O]);Q[N[O]](y);
								}
						}
						y.oW('pluginsLoaded');w(y);
					});
				});
			},
			w = function(y) {
			...
			x = function(y) {
				...
				a.gc.load(z, function() {
					...
					A.pathName = a.gc.getPath(z);y.oW('themeAvailable');
				});
			};
		a.application.prototype.iI = function(y) {
			...
			this.aL = {};
			this.aG = {};this.on('uiReady', function(B) {
				this.document.getWindow().on('lW', this.destroy, this);try {
					...
					if (this.cg.inPopup && (!g && !f.opera || C.$.top.location.href.match(/ckfinder.html/) || C.$.top.name == 'CKFinderpopup')) {
						...
						if (C.$.top.location.href.match(/ckfinder.html/) || C.$.top.name == 'CKFinderpopup') {
							this.document.focus();this.focus();
						}
						a.event.jP = true;this.document.on('keydown', function(J) {
							if (J.data.oV() == 116 || J.data.db() == a.bP + 82) {
								D.execCommand('refresh');C.$.event && C.$.event.keyCode && (C.$.event.keyCode = 5055);try {
			...
			this.cg = new d(this);this.on('configLoaded', function(B) {
				var C = this;
				e(C.cg, C, C.config.callback);
				C.id = C.config.id;
			}, this);s(this, A);a.oW('instanceCreated', null, this);
		};
		...
		destroy : function() {
			var o = this;
			o.theme.destroy(o);o.oW('destroy');a.remove(o);
		},
	...
	a.bF = '';(function() {
		...
		;a.aG = {
			...
			hS : function(r, s, t) {
				...
				a.log('[WIDGET] bX ' + u);
				this.bX[u] = new q(u, t);return this.bX[u];
			},
			bz : function(r, s, t, u, v) {
				...
				r.aG[w] = A;a.log('[WIDGET] instanced ' + w);return A;
			}
		};
		var p = {
				click : 1,
			...
			gA : function(r) {
				...
				for (var u = 0; u < s.cC.length; u++) {
					...
					if (p[v]) w.eh.on(v, s.fO, t);else {
						w.on(v, s.fO, t);w.app.on(v, s.fO, t);
					}
				}
			},
			ke : function(r) {
		...
		if (typeof o == 'object')
			...
			if (window.DOMParser)
				...
				if (p) {
					...
					p.validateOnParse = false;p.loadXML(q);
				}
			}
		}
		this.mq = p;
	...
	h.text.prototype = new h.bi();i.extend(h.text.prototype, {
		type : a.fl,
		hJ : function() {
	...
	h.pa = function(o) {
		...
		type : a.om,
		kA : function(o) {
			o = o.$;o.parentNode.insertBefore(this.$, o.nextSibling);
		}
	}, true, {
		append : 1,
		pd : 1,
		...
		o.appendTo(a.document.getHead());try {
	...
	CKFinder.lang.en = {
								...
								a.scriptLoader.load(B, function() {
									...
									for (var I = 0; I < H.length; I++)
										for (var J = 0; J < z.length; J++) {
											...
											if (K[H[I]]) {
												a.log('[PLUGIN] ' + K.name + '.' + H[I]);K[H[I]](q);
											}
									}
									q.cr('uiReady');q.cr('appReady');q.oW('pluginsLoaded', {
										step : 2,
										jN : q.connector
		...
		a.aL.Connector = function(q) {
			...
			if (q.config.connectorPath)
				this.oN = q.config.connectorPath;else
		...
		a.aL.Connector.prototype = {
			...
			sendCommand : function(q, r, s, t, u) {
				...
				if (s) return a.ajax.loadXml(v, function(x) {
						x.hy = w.app;w.app.oW('connectorResponse', {
							xml : x
						});s(i.extend(x, p));
			...
			sendCommandPost : function(q, r, s, t, u, v) {
				...
				if (t) return a.ajax.loadXml(w, function(y) {
						y.hy = x.app;x.app.oW('connectorResponse', {
							xml : y
						});t(i.extend(y, p));
			...
			composeUrl : function(q, r, s, t) {
				...
				if (q != 'Init') {
					...
					if (!t)
						t = x.app.aV;
					if (s)
						v = x.app.getResourceType(s).hash;else
		...
		var p = {
			checkError : function() {
				...
				w.hy.msgDialog('', t);return q != r.ERROR_UPLOADEDFILERENAMED;
			},
		...
		bM : [ 'connector' ],
		bz : function(p) {
			p.resourceTypes = [];p.on('connectorInitialized', function(q) {
		...
		a.aL.ResourceType.prototype = {
			isExtensionAllowed : function(q) {
				var r = this;
				q = q.toLowerCase();return (r.deniedExtensions.length === 0 || !r.ms[q]) && (r.allowedExtensions.length === 0 || !!r.oT[q]);
			},
		...
		m.add('folder', {
			...
			bz : function(s) {
				s.folders = [];
				s.aV = null;s.on('resourcesReceived', function y(t) {
					var u = [],
						v = t.data.hK;
				...
				if (s.eU) {
					s.dZ('folder0', 99);s.dZ('folder1', 100);s.dZ('folder2', 101);s.dZ('folder3', 102);s.eU({
						kl : {
		...
		function q(s, t, u, v, w) {
			...
			x.parent = s;
			x.nh = s.isRoot ? 0 : s.nh + 1;s.childFolders.push(x);return x;
		}
		;a.aL.Folder.prototype = {
			...
			getChildren : function(s, t) {
				var u = this,
					v = u.childFolders;
				if (u.hl && !t) {
					a.log('[FOLDER] getChildrenLock active, defering callback...');u.app.oW('requestLoadingFolder', {
						folder : u
					});
					var w = 100;
					setTimeout(function() {
						if (!u.hl) s(v);
						else if (w <= 3000) setTimeout(arguments.callee, w *= 2);else {
							a.log('[FOLDER] TIMEOUT for getChildrenLock defered callback!');
							u.hl = false;u.getChildren(s);
						}
					});return undefined;
				}
				if (v && !u.isDirty && !t) {
				...
				u.app.oW('requestLoadingFolder', {
					...
					u.isDirty = false;
					u.hl = null;u.app.oW('requestRepaintFolder', {
						folder : u
					});s(B);
				}, u.type, u);return null;
			},
			...
			createNewFolder : function(s) {
				r(s, this.app);
				var t = this;
				t.isDirty = true;t.app.connector.sendCommandPost('CreateFolder', {
					NewFolderName : s
				}, null, function(u) {
					if (u.checkError()) {
					...
					t.hasChildren = true;t.app.oW('afterCommandExecDefered', {
						name : 'CreateFolder',
						ip : t,
						uv : s
					});
			...
			rename : function(s) {
				...
				t.app.connector.sendCommandPost('RenameFolder', {
					...
					t.name = u.selectSingleNode('Connector/RenamedFolder/@newName').value;t.app.oW('requestRepaintFolder', {
						folder : t
					});
				}, this.type, this);
			},
			remove : function() {
				var s = this;
				s.deselect();
				s.parent.isDirty = true;this.app.oW('requestProcessingFolder', {
					folder : s
		...
		m.add('foldertree', {
			...
			bz : function z(x) {
				...
				x.on('themeSpace', function B(A) {
					...
					B.hX();f.opera && B.on('dblclick', function(E) {
						E.data.preventDefault();
					});
					var C = a.aG.bz(x, 'foldertree', y, B);
					if (x.bj) {
						x.bj.lX(B);x.bj.kh(function M(E, F) {
		...
		function q() {
			...
			x.dT.push(function() {
				...
				this.oW('beforeKeyboardNavigation', E, function M(G, H) {
					...
					try {
								...
								if (K.gE()) {
									...
									while (L.isOpened && L.hasChildren) {
										if (L.childFolders.length)
											L = L.childFolders[L.childFolders.length - 1];else break;
									}
									this.tools.cT(L);
						...
						this.oW('successKeyboardNavigation', H);this.oW('afterKeyboardNavigation', H);
					} catch (N) {
						N = a.ba(N);this.oW('failedKeyboardNavigation', H);this.oW('afterKeyboardNavigation', H);
						throw N;
					}
				});
				...
				this.oW('beforeRemoveFolder', C, function G(E, F) {
					...
					if (E) return;
					try {
						B.liNode().remove();H.oW('successRemoveFolder', F);H.oW('afterRemoveFolder', F);
					} catch (I) {
						H.oW('failedRemoveFolder', F);H.oW('afterRemoveFolder', F);
						throw a.ba(I);
					}
				});
			}, false);x.bh('LoadingFolder', 'requestLoadingFolder', function D(z) {
				var A = this,
					B = this.tools.cq(z);
				...
				this.oW('beforeLoadingFolder', C, function H(E, F) {
					...
					try {
						this.on('afterExpandFolder', function(I) {
							if (I.data && I.data.folder == G) {
								...
								if (J && J.hasClass('loading')) {
									J.remove();this.oW('requestRepaintFolder', {
										folder : G
									});
									F.step = 2;A.oW('successLoadingFolder', F);A.oW('afterLoadingFolder', F);
								}
							}
						}, null, null, 1);
						...
						F.step = 1;this.oW('successLoadingFolder', F);
					} catch (I) {
						this.oW('failedLoadingFolder', F);this.oW('afterLoadingFolder', F);
						throw a.ba(I);
					}
				});return undefined;
				...
				z.result = this.oW('beforeProcessingFolder', z.data, function F(B, C) {
					...
					try {
						var D = G.tools.cq(C.folder),
							E = D.aNode();
						E.addClass('processing');G.oW('successProcessingFolder', C);G.oW('afterProcessingFolder', C);
					} catch (H) {
						H = a.ba(H);G.oW('failedProcessingFolder', C);G.oW('afterProcessingFolder', C);
						throw H;
					}
				});
				...
				this.oW('beforeRepaintFolder', z.data, function J(B, C) {
					...
					try {
						...
						if (!D.hasChildren) {
							E.removeClass('openable');E.removeClass('closable');E.addClass('nochildren');F.removeAttribute('aria-expanded');
							if (H.$.hasChildNodes()) H.setHtml('');
						} else if (D.hasChildren)
							if (H.$.hasChildNodes()) {
								E.addClass('closable');E.removeClass('openable');F.setAttribute('aria-expanded', 'true');
							} else {
								E.addClass('openable');E.removeClass('closable');F.removeAttribute('aria-expanded');
						}
						K.oW('successRepaintFolder');K.oW('afterRepaintFolder');
					} catch (L) {
						K.oW('failedRepaintFolder');K.oW('afterRepaintFolder');
						throw a.ba(L);
					} return undefined;
				});
			});x.dT.push(function(z, A) {
					...
					try {
						if (G) {
							if (G.hasChildren === false) G.liNode().addClass('nochildren');else {
								...
								H = t(F, s);G.childrenRootNode().appendHtml(H);
							}
						} else {
							var J = {};
							...
							for (var L in J) {
								...
								if (L == -1) this.tools.kI().appendHtml(H);else {
									I = this.tools.cq(L);I.liNode().removeClass('nochildren');I.childrenRootNode().appendHtml(H);
								}
							}
				...
				if (B)
					if (C.hasClass('dropdown')) {
						z.jN.oW('contextmenu', z.data);z.cancel();return;
				}
					...
					this.oW('beforeSelectFolder', E, function K(G, H) {
						...
						try {
							...
							if (I) {
								...
								this.app.aV = I;A.tools.hk().select(I.aNode());
								if (H.jR) {
									A.oW('requestProcessingFolder', {
										folder : I
									});A.tools.mV(I, 1);A.app.oW('requestShowFolderFiles', {
										folder : I,
										mw : H.file
									}, function(L, M) {
										if (M.ib) M.ib.on('afterShowFolderFiles', function(N) {
												if (N.data.folder == I) {
													N.removeListener();A.oW('requestRepaintFolder', {
														folder : I
													});
												}
											});
								...
								this.oW('successSelectFolder');this.oW('afterSelectFolder');return I;
							}
						...
						try {
							if (J.app.aV) {
								...
								if (H) J.tools.hk().select(H);else {
									J.tools.hk().mc();J.oW('failedSelectFolderRefresh');
								}
							} else J.oW('successSelectFolderRefresh');
							J.oW('afterSelectFolderRefresh');return D;
						} catch (K) {
							J.oW('failedSelectFolderRefresh');J.oW('afterSelectFolderRefresh');
							throw a.ba(K);
						}
					});
			});x.dT.push(function(z, A) {
				...
				}, null, null, 999);A.on('successRemoveFolder', function() {
					A.oW('requestSelectFolderRefresh');
				...
				this.oW('beforeExpandFolder', E, function P(G, H) {
					...
					try {
						...
						if (I.hasChildren) {
							...
							if (L || M || O) {
								J.removeClass('openable');J.addClass('closable');K.setAttribute('aria-expanded', 'true');I.getChildren(function(Q) {
									if (Q) {
									...
									H.step = 2;A.oW('successExpandFolder', H);A.oW('afterExpandFolder', H);
								});
								H.step = 1;A.oW('successExpandFolder', H);
							} else if (H.hE || !H.hE && H.collapse) {
								J.removeClass('closable');J.addClass('openable');K.setAttribute('aria-expanded', 'false');I.childrenRootNode().setHtml('');
								I.isOpened = false;
								if (I.hm) I.getChildren(function(Q) {
										I.mapLoadedDescendants(function(R) {
											R.releaseDomNodes();
										});A.oW('successExpandFolder', H);A.oW('afterExpandFolder', H);
									});else {
			...
			x.tools.jL = function G(z, A, B) {
				...
				C.on('successExpandFolder', function(H) {
					...
					for (var K = 0; K < I.childFolders.length; K++) {
						...
						if (L.name == J)
							if (F.length === 0) {
								H.removeListener();B(L);return;
							} else {
								E = L;C.oW('requestExpandFolder', {
									folder : L
								});
						}
					}
			...
			x.tools.mV = function(z, A) {
				var B = z.type,
					C = z.getPath(),
					D = this.ib.app.id;
				A = A === undefined ? z.isOpened : !!A + 1 - 1;i.setCookie(D ? 'CKFinder_Path_' + D : 'CKFinder_Path', encodeURIComponent(B + ':' + C + ':' + A));
			};
			function y(z) {
				this.ib = z;
		...
		function r() {
			i.extend(a.aL.Folder.prototype, {
				...
				focus : function() {
					var x = this.aNode();
					if (x) {
						x.setAttribute('tabindex', 0);x.focus();
					}
				},
				blur : function() {
					var x = this.aNode();
		...
		function w(K) {
			...
			N.call(M, K.qX());K.bn().addClass('files_' + v[0]);
		}
		;
		function x(K) {
		...
		;m.add('filesview', {
			bM : [ 'foldertree' ],
			onLoad : function K() {
				F();z();a.dialog.add('moveFileExists', function(L) {
					return {
			...
			bz : function M(K) {
				...
				K.rQ.jf = q.fX;K.on('themeSpace', function P(N) {
					...
					K.config.selectMultiple && O.on('click', function(R) {
						...
						P.tools.cR(T, true);R.cancel();R.data.preventDefault();
					}, null, null, 1);K.bD('ViewFile', {
						exec : function(R) {
							var S = P.data().cG;
							if (S) {
						...
						exec : function(R) {
							...
							if (S) {
								...
								if (R.config.directDownload)
									T = S.folder.getUrl() + S.name + '?download';else
						...
						exec : function(R) {
							var S = function(U, V) {
									try {
										...
										if (W instanceof a.dU) R.msgDialog('', W.message);else
											throw W;
									}
								},
								T = P.data().cG;
					...
					if (K.eU) {
						K.dZ('file0', 90);K.dZ('file1', 100);K.dZ('file2', 110);K.dZ('file3', 120);K.eU({
							selectFile : {
								label : K.lang.Select,
								onClick : function() {
					...
					if (K.bj) {
						K.bj.lX(O);K.bj.kh(function W(R, S) {
							var T = P.tools.bZ(R);
							if (T) {
		...
		function z() {
			...
			function O() {
				...
				R._.nV = true;var Q = P.substr(1, 1);
				switch (Q) {
				case 'D':
					R.data().cN = 'date';
					break;case 'S':
			...
			;K.dT.push(O);K.dT.push(function() {
				i.mH(this.bn());
				...
				Q.oW('beforeSelectAction', T, function gs(V, W) {
					...
					try {
						...
						if (X) {
							...
							if (W.jw) {
								...
								if (Q.app.config.selectThumbnailActionFunction)
									bW.selectThumbnailActionData = Q.app.config.selectThumbnailActionData;else
						...
						Q.oW('successSelectAction', W);Q.oW('afterSelectAction', W);
					} catch (am) {
						am = a.ba(am);Q.oW('failedSelectAction', W);Q.oW('afterSelectAction', W);
						throw am;
					}
				});
					...
					try {
						...
						if (aT > a.dy && fv.app.config.selectMultiple) {
							...
							for (var bW = 0; bW < bm.length; bW++) {
								if (!bm[bW].isDeleted) {
									Y = bm[bW].rowNode();break;
								}
							}
							do Q.tools.bZ(Y).select(true); while ((Y = Y.dG()) && Y.ge());
							Q.tools.cR(Q.tools.bZ(Y), true);P.data.preventDefault();
						}
						if (R && aT >= 37 && aT <= 40) {
							var bm = Q.data().cP[Q.data().cN] || Q.data().files;
							for (var bW = 0; bW < bm.length; bW++) {
								if (!bm[bW].isDeleted) {
									fv.tools.cR(bm[bW], aa);break;
								}
							}
						} else {
							if (Q.data().dA == 'list') {
						...
						fv.oW('successKeyboardNavigation', X);fv.oW('afterKeyboardNavigation', X);
					} catch (aP) {
						aP = a.ba(aP);fv.oW('failedKeyboardNavigation', X);fv.oW('afterKeyboardNavigation', X);
						throw aP;
					}
				});
					...
					try {
						...
						if (!V) this.oW('failedProcessingFile', U);else {
							...
							this.on('afterProcessingFile', function(Y) {
								...
								U.step = 2;this.oW('successProcessingFile', U);this.oW('afterProcessingFile', U);Y.removeListener();
							});
							U.step = 1;this.oW('successProcessingFile', U);
						}
					} catch (Y) {
						this.oW('failedProcessingFile', U);this.oW('afterProcessingFile', U);
						throw a.ba(Y);
					}
				});
				...
				this.oW('beforeRepaintFile', R, function Y(T, U) {
					...
					try {
						...
						Z.oW('failedRepaintFile', U);Z.oW('afterRepaintFile', U);
						throw a.ba(aa);
					}
				});
			});
			if (g && f.ie6Compat && !f.ie7Compat && !f.ie8) K.bh('HoverFile', [ 'mouseover', 'mouseout' ], function S(P) {
					...
					this.oW('beforeHoverFile', R, function V(T, U) {
						...
						try {
							...
							W.oW('successHoverFile', U);W.oW('afterHoverFile', U);
						} catch (X) {
							W.oW('failedHoverFile', U);W.oW('afterHoverFile', U);
							throw a.ba(X);
						}
					});
				});
			K.bh('RenderThumbnails', [ 'requestRenderThumbnails' ], function eS(P) {
				...
				if (!Q.files.length)
					...
					for (U = 0; U < Q.files.length; U++) {
						R[U] = Q.files[U];R[U].releaseDomNodes();
					}
				...
				this.oW('beforeRenderFiles', aa, function bV(eS, fv) {
					...
					p = a.bF.substr(7, 1);try {
						for (var aP = 0; aP < fv.files.length; aP++) fv.files[aP].releaseDomNodes();
						this.tools.cR();this.oW('requestAddFiles', fv, function(eN) {
							if (!eN)
								Q.hA = fv.dA;
						});this.oW('successRenderFiles', fv);this.oW('afterRenderFiles', fv);
					} catch (eN) {
						this.oW('failedRenderFiles', fv);this.oW('afterRenderFiles', fv);
						throw a.ba(eN);
					}
				});
				...
				Q = this;P.on('afterCommandExecDefered', function(R) {
					if (!R.data) return;
					var S = R.data.name,
						T;
					if (S == 'RenameFile') {
						...
						Q.oW('requestRenderFiles', {
							...
							if (W) return;
							U.deselect(true);Q.oW('requestSelectFile', {
								file : R.data.file,
								multiple : V
							}, function() {
								if (W) return;
						...
						T = R.data.folder;
						if (Q.tools.currentFolder() != T) return;
						Q.tools.cR();Q.bn().focus();Q.oW('requestRenderFiles', {
							folder : T,
							lK : 1
						});
					}
				...
				if (R)
					if (P.data.bK().hasClass('dropdown')) {
						P.jN.oW('contextmenu', P.data);P.cancel();return;
				}
				var T = i.extend({}, P.data, {
					file : Q
				}, true);
				this.oW('beforeSelectFile', T, function aT(V, W) {
					...
					try {
						...
						if (X && S && X.rowNode().hasClass('selected') && bm.tools.dH()) {
							...
							if (!bm.data().nY)
								bm.data().nY = [];
							bm.data().cG = X;bm.data().nY.push(X);R && bm.tools.cR(X, S, S);
						} else if (bm.tools.dH() && !S) {
							bm.data().cG = null;
							bm.data().nY = [];bm.tools.cR();
						}
						bm.oW('successSelectFile', W);bm.oW('afterSelectFile', W);
					} catch (bW) {
						bm.oW('failedSelectFile', W);bm.oW('afterSelectFile', W);
						throw a.ba(bW);
					}
				});
				...
				this.oW('beforeAddFiles', Q, function eS(S, T) {
					...
					try {
						...
						if (T.dA == 'list') {
							...
							V.removeClass('files_thumbnails');V.addClass('files_details');
						...
						if ((T.eu && !Y || X) && T.mj) {
							V.addClass('files_message');this.tools.of().setHtml(T.mj);
						}
						if (!p && !X) Z.setHtml('');
				...
				this.oW('beforeShowFolderFiles', S, function Y(U, V) {
					...
					this.app.cS('refresh').bR(a.aY);try {
						...
						P.data.ib = this;
						this.data().folder = W;Q.tools.kR();this.oW('requestRenderFiles', {
							eu : 1,
							mj : Q.app.lang.FilesLoading
						});this.app.connector.sendCommand('GetFiles', X, function(Z) {
							Q.app.cS('refresh').bR(a.aS);
							if (Q.app.aV != W) {
								Q.oW('failedShowFolderFiles', V);Q.oW('afterShowFolderFiles', V);return;
							}
							...
							if (aa) {
								Q.app.oW('requestSelectFile', {
									...
									aa.aNode().$.scrollIntoView(1);Q.app.layout.mB().getFirst().$.scrollIntoView(0);
								}, f.opera ? 500 : 100);
							}
							Q.oW('successShowFolderFiles', V);Q.oW('afterShowFolderFiles', V);x(Q);
						}, W.type, W);
					} catch (Z) {
						this.oW('failedShowFolderFiles', V);this.oW('afterShowFolderFiles', V);
						throw a.ba(Z);
					}
				});
			});
			...
			K.tools.lz = function() {
				...
				var R = Q.length - 1;
				if (R)
					Q[R] = '<td class="last">' + Q[R].substr(4);else
			...
			K.tools.qc = function(P) {
				...
				var U = T.length - 1;
				if (U)
					T[U] = '<td class="last">' + T[U].substr(4);else
			...
			K.tools.dH = function() {
				var P = this.ib.data();
				if (P.cG)
					if (!P.cG.isDeleted) return P.cG;else return P.cG = null;
			};
			...
			K.tools.downloadFile = function(P, Q) {
				...
				if (!R) {
					R = P.createElement('iframe');R.setAttribute('id', 'downloadIframe');R.setStyle('display', 'none');P.bH().append(R);
				}
				...
				if (aa == W.ERROR_COPY_FAILED || aa == W.ERROR_MOVE_FAILED) {
					...
					for (bW = 0, eS = aP.length; bW < eS; bW++) {
						...
						if (dX == W.ERROR_ALREADYEXIST)
							eN = 1;else gB.push([ aP[bW].getAttribute('name'), K.lang.Errors[dX] ]);
					...
					if (!bV && !eN) K.msgDialog('', K.lang.Errors[M ? 300 : 301] + '<br /><br />' + C(gB), P || null);
						...
						fv += K.lang[M ? 'MoveFailedList' : 'CopyFailedList'].replace('%s', C(gB));K.msgDialog('', fv, P || null);
					}
					...
					eN && K.cg.openDialog('moveFileExists', function(aq) {
						...
						fv += '<strong>' + as + '</strong>';aq.show();aq.getContentElement('tab1', 'msg').getElement().setHtml(fv);aq.on('ok', function az(at) {
							at.removeListener();
							...
							case 'overwrite':
								...
								break;case 'skip':
								if (!av) {
									aT.shift();aT.length && setTimeout(function() {
					...
					X = 1;
				if (X) {
					L.errorCallback && L.errorCallback();return;
				}
				var gR = B(K, bm, L.fileCallback);
				if (gR) {
					fv = K.lang.OperationCompletedSuccess + '<br />' + K.lang[M ? 'MovedFilesNumber' : 'CopiedFilesNumber'].replace('%s', gR);K.msgDialog('', fv, P || null);
				} else P && P();
		...
		a.aL.File.prototype = {
			rename : function(K) {
				...
				L.app.oW('requestProcessingFile', {
					...
					L.thumb = 0;L.app.oW('afterCommandExecDefered', {
						name : 'RenameFile',
						file : L
					});
			...
			focus : function(K, L) {
				...
				M.setAttribute('tabindex', 0);M.focus();
			},
			blur : function() {
		...
		function E(K, L) {
			...
			for (var P = 0, Q = L.length; P < Q; P++) {
				...
				M['files[' + P + '][folder]'] = O;K.oW('requestProcessingFile', {
					file : L[P]
				});
			}
			K.connector.sendCommandPost('DeleteFiles', null, M, function(R) {
				...
				for (T = 0, U = L.length; T < U; T++) {
					if (!S[L[T]]) {
						L[T].isDeleted = true;L[T].releaseDomNodes();
					}
				}
		...
		;
		function q(x) {
			x += '';var y = x.charAt(0).toUpperCase();
			return y + x.substr(1);
		}
		;
		function r(x) {
				...
				if (B.type == 'fileButton' && B['for']) {
					...
					if (!F) {
						...
						B.onShow = function(I) {
							...
							if (!M) return false;
							M._.dg.action = F;M.reset();return true;
						};
					} else {
						B.filebrowser.url = F;
						B.hidden = false;s(x, z.vz(B['for'][0]).eB(B['for'][1]), B.filebrowser);
					}
					var H = B.onClick;
		...
		;m.add('filebrowser', {
			...
			for (var A in y.contents) {
				z = y.contents[A];t(x.application.cg, x.data.name, y, z.elements);
	...
	n.button._ = {
		...
		keydown : function(p, q) {
			...
			if (r.onkey) {
				q = new h.event(q);return r.onkey(r, q.db()) !== false;
			}
		},
		focus : function(p, q) {
			var r = n.button._.instances[p],
	...
	n.prototype.qW = function(p, q) {
		...
		m.add('container', {
			...
			pV : function(p) {
				...
				function y(A) {
					A && A.removeListener();var B = x.getFrameDocument().$;
					B.open();
					if (v)
						B.domain = document.domain;
					p.document = new j(B);p.theme.dQ(p);B.close();
					(B.defaultView || B.parentWindow).CKFinder = CKFinder;a.skins.load(p, 'application', function() {
						var D = p.dJ;
				...
				if (g && f.version >= 9 && p.cg.inPopup)
					p.element.getDocument().getWindow().$.ckfinder = function() {
						p.element.getDocument().getWindow().$.ckfinder = undefined;y();
					};
				x.on('load', y);
				...
				if (f.gecko) {
					...
					x.setAttribute('title', z);x.setAttribute('name', z);
			...
			p.bj = new m.bj(p);p.bD('bj', {
				exec : function() {
					...
					if (q.hasClass('focus_inside')) {
						...
						if (u) {
							r = u.dM;
							s = r.ir();p.bj.show(p.document.bH().getParent(), null, s.x + 5, s.y + 5, r, q);
							p._.activeElement = r;
							p._.oO = t.tools.oO();return;
						}
					}
					q = p.layout.pS();
					if (q.hasClass('focus_inside')) {
	...
	m.bj = i.createClass({
		$ : function(p) {
			...
			this._.vx = i.addFunction(function(q) {
				this._.panel.hide();p.focus && p.focus();
			}, this);
		},
		_ : {
			onMenu : function(p, q, r, s, t, u) {
				...
				v.onEscape = function() {
					w.focus && w.focus();t.focus && t.focus();
					w._.activeElement = null;
				};
				var x = this._.dF,
					y = [];
				v.onHide = i.bind(function() {
					v.onHide = null;this.onHide && this.onHide();
				}, this);
				for (var z = 0; z < x.length; z++) {
					var A = x[z];
		...
		ej : {
			lX : function(p, q) {
				if (f.opera && !('oncontextmenu' in document.body)) {
					...
					p.on('mousedown', function(v) {
						...
						if (r) {
							r.remove();
							r = undefined;p.oW('contextmenu', v.data);
						}
					});
				}
				...
				if (f.webkit) {
					...
					p.on('keydown', t);p.on('keyup', u);p.on('contextmenu', u);
				}
			},
			kh : function(p, q) {
				this._.dF.push([ p, q ]);
			...
			show : function(p, q, r, s, t, u) {
				this.app.focus();this._.onMenu(p || a.document.gT(), q, r || 0, s || 0, t, u);
			}
		}
	});(function() {
		m.add('dragdrop', {
			...
			gr : function u(r) {
				...
				r.on('themeSpace', function w(v) {
					...
					var w;
					r.aG['filesview.filesview'].gA('Draggable');r.aG['foldertree.foldertree'].ke('Droppable');
					...
					if (!x.rowNode().hasClass('selected'))
						if ((v.data.$.ctrlKey || v.data.$.metaKey) && r.config.selectMultiple) y.push(x);else
							y = [ x ];
					var z = i.extend({}, {
						file : x,
						files : y,
					...
					w.oW('beforeDraggable', z, function J(B, C) {
						...
						var D = x.rowNode(),
							E = 0,
							F = 0;
						s = s || r.document.getById('dragged_container');s.hide();r.document.on('mousemove', G);
						function G(K) {
							s.setStyles({
							...
							w.app.cK.kG(D);w.app.cK.kz(y);
							for (var L = 0, M = y.length; L < M; L++) y[L].rowNode().addClass('dragged_source');
							if (y.length == 1) {
								s.setStyle('width', D.rd('width'));s.addClass('file_entry');
							...
							if (y.length == 1) {
								...
								N = q(w, y);
							s.setHtml(N);
							F = 1;w.app.document.bH().addClass('dragging');w.app.aG['foldertree.foldertree'].gA('Droppable');
							C.step = 1;w.oW('successDraggable', C);
						}
						;
						function H(K) {
							s.hide();s.removeClass('drag_multiple');s.removeClass('file_entry');s.setStyle('width', 'auto');s.setHtml('');
							for (var L = 0, M = y.length; L < M; L++) y[L].rowNode().removeClass('dragged_source');
							w.app.cK.kG(null);w.app.cK.kz(null);r.document.removeListener('mousemove', G);
							if (K) K.removeListener();else r.document.removeListener('mouseup', H);
							w.app.aG['foldertree.foldertree'].ke('Droppable');w.app.document.bH().removeClass('dragging');
							C.step = 2;w.oW('successDraggable', C);w.oW('afterDraggable', C);
						}
					...
					if (y == 'mouseup') {
						...
						x.oW('beforeDroppable', C, function N(E, F) {
							...
							try {
										...
										onClick : function() {
											x.oW('successDroppable', {
												...
												errorCallback : function() {
													x.oW('failedDroppable', F);x.oW('afterDroppable', F);
												}
											});
										}
									}),
								...
								O = a.ba(O);x.oW('failedDroppable', F);x.oW('afterDroppable', F);
								throw O;
							}
						});
					} else if (y == 'mouseover') {
		...
		;p.prototype = {
			iW : function(r) {
				...
				var s = !!r;
				if (s && !t.fZ) {
					t.app.document.bH().addClass('drop_accepted');r.addClass('drop_target');
				} else if (!s && t.fZ) {
					t.app.document.bH().removeClass('drop_accepted');t.fZ.removeClass('drop_target');
				}
				t.fZ = s ? r : null;
			},
			kG : function(r) {
		...
		function r(s, t, u, v, w) {
			...
			if (!y) {
				...
				y.element = u.append(k.kE(y.nt(s), u.getDocument()));y.element.setStyles({
					display : 'none',
					position : 'absolute'
				});
			}
		...
		;n.pY = i.createClass({
			...
			ej : {
				...
				gf : function(s, t, u, v, w) {
					...
					if (u == 3 || u == 4)
						F += t.$.offsetHeight - 1;
					this._.panel._.nr = t.dS();z.setStyles({
						top : F + 'px',
						left : '-3000px',
						visibility : 'hidden',
						opacity : '0',
					...
					if (!this._.qa) {
						...
						a.event.jP = true;G.on('blur', function(H) {
							if (g && !this.fj()) return;
							var I = H.data.bK(),
					...
					x.onEscape = i.bind(function() {
						...
						if (y.oz) {
							function I() {
								...
								R += 4;O.setStyle('width', R + 'px');y.element.addClass('cke_frameLoaded');var W = y.element.$.scrollHeight;
								if (g && f.quirks && W > 0)
									W += (O.$.offsetHeight || 0) - (O.$.clientHeight || 0);
								O.setStyle('height', W + 'px');x._.iL.element.setStyle('display', 'none').removeStyle('display');
							}
							;
							if (x.hm) I();else
								x.onLoad = I;
						...
						z.setStyles({
							...
							if (B.ny)
								if (f.gecko) {
									var O = A.getParent();
									O.setAttribute('role', 'region');O.setAttribute('title', B.ny);A.setAttribute('role', 'region');A.setAttribute('title', ' ');
							}
				...
				hide : function() {
					...
					if (s.visible && (!s.onHide || s.onHide.call(s) !== true)) {
						s.gU();s.element.setStyle('display', 'none');
						s.visible = 0;
					}
				},
				fj : function(s) {
				...
				rA : function(s, t, u, v, w, x) {
					...
					this._.gF = s;
					this._.lG = false;s.gf(t, u, v, w, x);
		...
		function p() {
			...
			w.dT.push(function() {
				...
				x.result = this.oW('beforeUnloadForm', function D(z, A) {
					...
					if (z) return;
					try {
						E.bn().getParent().setStyle('display', 'none');E.app.layout.ea(true);
						if (E.data().dc) {
						...
						E.tools.releaseDomNodes();E.oW('successUnloadForm', A);
					} catch (F) {
						E.oW('failedUnloadForm', A);E.oW('afterUnloadForm', A);
						throw a.ba(F);
					}
				});
				...
				x.result = this.oW('beforeLoadForm', z, function I(B, C) {
					...
					try {
						var D = this.bn();
						D.setHtml(C.html);D.getParent().removeStyle('display');this.app.layout.ea(true);var E = this.tools.ij();
						if (E) {
							if (C.dq)
								if (C.cC)
							...
							for (var F = 0; F < G.count(); F++) {
								if (G.getItem(F).getAttribute('name') == 'cancel') {
									G.getItem(F).on('click', function(J) {
										y.oW('requestUnloadForm');J.removeListener();
									});break;
								}
							}
							if (C.cancelSubmit) E.on('submit', s);
				...
				x.result = this.oW('beforeFilesViewSettingsForm', {}, function D(z, A) {
					...
					try {
						if (this.data().dc == 'settings') this.oW('requestUnloadForm', function() {
								this.oW('successFilesViewSettingsForm', A);this.oW('afterFilesViewSettingsForm', A);
							});else {
							if (this.data().dc) this.oW('requestUnloadForm');
							...
							this.oW('requestLoadForm', {
								...
								this.eh.addClass('show_border');this.app.cg.resizeFormPanel();this.oW('successFilesViewSettingsForm', A);
							});
						}
					} catch (E) {
						this.oW('failedFilesViewSettingsForm', A);this.oW('afterFilesViewSettingsForm', A);
						throw a.ba(E);
					}
				});
			});
		...
		function q(w) {
			if (w.name == 'submit') {
				var x = this.app.aG['formpanel.formpanel'],
					y = x.data();
				this.oW('requestUnloadForm');this.oW('afterFilesViewSettingsForm', y);return;
			}
			var z = w.data.bK(),
	...
	a.dJ = function(p) {
		...
		var p,
			...
			r = function(s) {
				if (p) {
					p = false;s.data.preventDefault(true);
				}
			};
		a.dJ.prototype = {
			oA : function(s) {
	...
	l.gN = [];
	l.keystrokes = [ [ a.eJ + 119, 'foldertreeFocus' ], [ a.eJ + 120, 'filesviewFocus' ], [ a.eJ + 121, 'hW' ], [ a.eJ + 85, 'upload' ], [ a.dy + 121, 'bj' ], [ a.bP + a.dy + 121, 'bj' ] ];m.add('menu', {
		eK : function(p) {
			...
			ej : {
				...
				show : function(q, r, s, t) {
					...
					if (!w) {
						...
						z[32] = 'click';
						z[39] = 'click';
						x = this._.element = y.element;x.addClass(v.iy);
						var A = x.getDocument();
						A.bH().setStyle('overflow', 'hidden');A.eG('html').getItem(0).setStyle('overflow', 'hidden');
	...
	n.panel.prototype = {
		...
		er : function(p, q) {
			...
			if (u.lE || u.css.length) {
				...
				q.push(g ? 'javascript:void(function(){' + encodeURIComponent(t) + '}())' : '');q.push('"></iframe>');
			}
			q.push('</div></div>');return r;
		},
		oU : function() {
			var p = this._.rE;
			if (!p) {
				if (this.lE || this.css.length) {
					...
					x.$.CKFinder = CKFinder;v.on('keydown', function(z) {
						var B = this;
						var A = z.data.db();
						if (B._.onKeyDown && B._.onKeyDown(A) === false) {
							z.data.preventDefault();return;
						}
		...
		gf : function(p) {
			...
			t._.iL = r;
			r._.cQ = -1;
			t._.onKeyDown = r.onKeyDown && i.bind(r.onKeyDown, r);r.show();return r;
		}
	};
		...
		ej : {
			...
			onKeyDown : function(p) {
				...
				case 'next':
					...
					r = u._.cQ;t = r >= 0 && u.element.eG('a').getItem(r);
					if (t)
		...
		bz : function(p) {
			...
			if (q.nB) p.on('uiReady', function() {
					var r = null,
						...
						t;
					function u(w) {
						p.document.bH().addClass('during_sidebar_resize');var x = w.data.$.screenX - s.x,
							y = t.width + x * (p.lang.dir == 'rtl' ? -1 : 1);
						p.nJ(Math.max(q.nN, Math.min(y, q.nC)));
					}
					;
					function v(w) {
						p.document.bH().removeClass('during_sidebar_resize');a.document.removeListener('mousemove', u);a.document.removeListener('mouseup', v);
						if (p.document) {
							p.document.removeListener('mousemove', u);p.document.removeListener('mouseup', v);
						}
					}
					;p.layout.dV().on('mousedown', function(w) {
						...
						s = {
							x : w.data.$.screenX
						};a.document.on('mousemove', u);a.document.on('mouseup', v);
						if (p.document) {
							p.document.on('mousemove', u);p.document.on('mouseup', v);
						}
					});
				});
		}
				...
				s.on('themeSpace', function w(v) {
					...
					if (x.app == s) {
						x.on('successSelectFile', function B(A) {
							...
							A.data.ib = this;y.oW('requestShowFolderInfo', A.data);
						});
					}
			...
			s.bh('ShowFileInfo', [ 'requestShowFileInfo' ], function u(t) {
				t.result = this.oW('beforeShowFileInfo', t.data, function z(v, w) {
					...
					try {
						...
						A.bn().setHtml(y);A.oW('successShowFileInfo', w);
					} catch (B) {
				...
				t.result = this.oW('beforeShowFolderInfo', t.data, function z(v, w) {
					...
					try {
						...
						A.bn().setHtml(y);A.oW('successShowFolderInfo', w);
					} catch (B) {
			...
			bz : function(r) {
				var s = function(t, u) {
					...
					case r.lang.dir == 'rtl' ? 37:
						...
						return false;case r.lang.dir == 'rtl' ? 39:
							37:
				...
				r.on('themeSpace', function(t) {
					if (t.data.space == 'mainTop') {
						...
						var u = 'cke_' + i.getNextNumber(),
							...
							w;
						v.push('>');v.push('<span id="', u, '" class="cke_voice_label">', r.lang.toolbar, '</span>');
						var x = r.dh.fk,
						...
						for (var z = 0; z < y.length; z++) {
							...
							if (A === '/') {
								v.push('<div class="cke_break"></div>');continue;
							}
							v.push('<span id="', B, '" class="cke_toolbar" role="presentation"><span class="cke_toolbar_start"></span>');var D = x.push(C) - 1;
							if (D > 0) {
								C.previous = x[D - 1];
								C.previous.next = C;
							}
	...
	n.separator = {
		er : function(p, q) {
			q.push('<span class="cke_separator"></span>');return {};
		}
	};
	...
	l.toolbar = 'Full';(function() {
		m.add('tools', {
			...
			addTool : function(p, q) {
				...
				p = q ? '<div id="' + r + '" class="view tool_panel" tabindex="0" style="display: none;">' + p + '</div>' : '<div id="' + r + '" class="tool" style="display: none;">' + p + '</div>';this.app.layout.dV().getChild(0).appendHtml(p);return r;
			},
			...
			hideTool : function(p) {
				this.app.document.getById(p).setStyle('display', 'none');this.app.layout.ea(true);
			},
			showTool : function(p) {
				this.app.document.getById(p).removeStyle('display');this.app.layout.ea(true);
			},
			removeTool : function(p) {
				this.hideTool(p);this.app.document.getById(p).remove();
			}
		});
	})();(function() {
		m.add('uploadform', {
			...
			md : function() {
				...
				q.setAttribute('type', 'file');return q.disabled === false;
			},
			onLoad : function q() {
				if (!this.md()) return;
				p();
		...
		function p() {
					...
					this.oW('beforeUploadFileForm', {
						...
						try {
							if (A.dc == 'upload') this.oW('requestUnloadForm', function() {
									this.app.cS('upload').bR(a.aS);this.oW('successUploadFileForm', F);this.oW('afterUploadFileForm', F);
								});else {
								...
								this.oW('requestLoadForm', {
									...
									F.step = 1;this.oW('successUploadFileForm', F);
								});
								function K(N) {
									if (N.data.folder && N.data.folder.acl.fileUpload) {
										var O = w.tools.qO();
										w.oW('requestUnloadForm');w.oW('requestUploadFileForm', function Q() {
											var P = w.tools.qO();
											O.kB(P);P.remove();
											delete w.tools.jj;
										});
								...
								L.on('successShowFolderFiles', K);this.on('requestUnloadForm', function O(N) {
									N.removeListener();L.removeListener('successShowFolderFiles', K);
								});
							}
						} catch (N) {
							this.oW('failedUploadFileForm', F);this.oW('afterUploadFileForm', F);
							throw a.ba(N);
						}
					});
				});
			...
			function s(u, v) {
				var w = this,
					...
					A = z && z.$.value;
				if (!A.length) {
					u.data.preventDefault(true);this.oW('failedUploadFileForm');this.oW('afterUploadFileForm');return false;
				}
				var B = A.match(/\.([^\.]+)\s*$/)[1];
				if (!B || !v.getResourceType().isExtensionAllowed(B)) {
					u.data.preventDefault();w.app.msgDialog('', w.app.lang.UploadExtIncorrect);
				} else
					y = 0;
				if (y) {
					u.data.preventDefault(true);this.oW('failedUploadFileForm');this.oW('afterUploadFileForm');return false;
				...
				C.OnUploadCompleted = function(D, E) {
					...
					if (E && !D) {
						w.app.msgDialog('', E);
						var G = w.tools.qB();
						G.setStyle('display', 'none');G.getChild(1).setText('');G.getChild(2).setText('');w.oW('failedUploadFileForm', F);
					} else {
						...
						w.oW('requestUnloadForm');w.oW('successUploadFileForm', F);
					}
					w.oW('afterUploadFileForm', F);try {
		...
		function p(q, r) {
						...
						if (Q.ie && Q.win) {
							A.attachEvent(y, function() {
								if (A.readyState == 'complete') {
									A.detachEvent(y, arguments.callee);S();
								}
							});
							if (z == top) (function() {
									if (L) return;
			...
			function W() {
				var pw = A.getElementsByTagName('body')[0],
					aq = eN(t);
				aq.setAttribute('type', w);var ar = pw.appendChild(aq);
				if (ar) {
					var as = 0;
					(function() {
						if (typeof ar.GetVariable != s) {
							...
							as++;setTimeout(arguments.callee, 10);return;
						}
						pw.removeChild(aq);
						ar = null;X();
					})();
			...
			function aa(pw, aq, ar, as) {
				...
				if (at) {
					...
					if (typeof aq.flashvars != s)
						aq.flashvars += '&' + av;else
						aq.flashvars = av;
					if (Q.ie && Q.win && at.readyState != 4) {
						var aw = eN('div');
						ar += 'SWFObjectNew';aw.setAttribute('id', ar);at.parentNode.insertBefore(aw, at);
			...
			function aT(pw) {
				if (Q.ie && Q.win && pw.readyState != 4) {
					var aq = eN('div');
					pw.parentNode.insertBefore(aq, pw);aq.parentNode.replaceChild(bm(pw), aq);
			...
			function bm(pw) {
				var aq = eN('div');
				if (Q.win && Q.ie)
					aq.innerHTML = pw.innerHTML;else {
			...
			function eS(pw, aq, ar) {
				var as = eN('param');
				as.setAttribute('name', aq);as.setAttribute('value', ar);pw.appendChild(as);
			}
			;
			function fv(pw) {
				var aq = bV(pw);
			...
			function dX(pw) {
				...
				ar[2] = parseInt(ar[2], 10) || 0;return aq[0] > ar[0] || aq[0] == ar[0] && aq[1] > ar[1] || aq[0] == ar[0] && aq[1] == ar[1] && aq[2] >= ar[2] ? true : false;
			}
			;
			function gs(pw, aq, ar, as) {
				...
				if (!N || O != au) {
					var av = eN('style');
					av.setAttribute('type', 'text/css');av.setAttribute('media', au);
					N = at.appendChild(av);
			...
			function am(pw, aq) {
				...
				if (L && bV(pw))
					bV(pw).style.visibility = ar;else gs('#' + pw, 'visibility:' + ar);
			}
			;
			function gP(pw) {
				var aq = /[\\\"<>\.;]/,
					...
					if (Q.w3 && !(Q.wk && Q.wk < 312) && pw && aq && ar && as && at) {
						am(aq, false);T(function() {
							...
							if (dX(at)) {
								...
								aA.data = au;aa(aA, aC, aq, ay);return;
							} else am(aq, true);
							if (ay) ay(az);
						});
					} else if (ay) ay(az);
		...
		function s(w, x) {
			...
			if (z.bu == a.aY) return;
			if (z.bu == a.eV)
				for (A = 0; A < x.length; A++) v(y, x[A]);else {
			...
			bz : function y(w) {
				...
				setTimeout(function() {
					w.cg.replaceUploadForm(x, function() {
						...
						q = t(D.getById('ckf_globalUploads'), w);E.on('change', function(G) {
							s(w, G.jN.$.files);
						});
					...
					var A = function(B) {
						...
						if (C.files && C.files.length || C.types && (C.types.contains && C.types.contains('Files') || C.types.indexOf && C.types.indexOf('Files') != -1)) {
							C.dropEffect = 'copy';B.data.stopPropagation();B.data.preventDefault();
						} else {
							C.dropEffect = 'none';B.data.preventDefault();
						}
					};
					z.on('dragover', A);z.getDocument().on('drop', function(B) {
						var C = B.data.$.dataTransfer;
						if (C && C.files && C.files.length > 0) {
							s(w, C.files);B.data.preventDefault();
						}
					});
				}, 500);
			}
		...
		function t(w, x) {
			w = w.getChild([ 0, 0 ]);var y = w.getChild(0),
				z = w.getChild(1),
				A = w.getChild([ 2, 0 ]),
				B = w.getChild(3),
				C = w.getChild(4),
			...
			this.ld = function(Q) {
				D[Q.id] = Q;F++;
				if (F == 1) {
					G.getFirst().setHtml(x.lang.UploadBtnCancel);G.addClass('cke_dialog_ui_button_cancel');G.removeClass('cke_dialog_ui_button_ok');
				}
			};
			this.lQ = function(Q) {
				E--;
			...
			this.mU = function(Q, R, S) {
				...
				if (F === 0) {
					G.getFirst().setHtml(x.lang.CloseBtn);G.addClass('cke_dialog_ui_button_ok');G.removeClass('cke_dialog_ui_button_cancel');O();
				}
				or();nU();
			};
			...
			this.or = function() {
				...
				y.setText(x.lang.UploadTotalFiles + ' ' + L + ' ');z.setText(x.lang.UploadTotalSize + ' ' + CKFinder.tools.formatSize(M / 1024, x.lang));
			};
			this.nv = function(Q) {
				L++;
				M += Q.size;P();
		...
		function u(w, x) {
			...
			this.speed = 0;
			this.lT = 0;
			this.nZ = 0;B.setAttribute('class', 'ckf_upload_info');B.setHtml('<div class="ckf_progress_wrapper"><div class="ckf_progress_info"><span>' + x + ' </span>' + '<span></span>' + '<div class="ckf_progress_bar_container"><div></div></div>' + '<span class="ckf_status"></span>' + '<span class="ckf_speed"></span>' + '<div class="ckf_outcome"></div>' + '</div></div>' + '<div class="ckf_uploadButtons">' + '<a class="cke_dialog_ui_button cke_dialog_ui_button_cancel" name="cancel"><span class="cke_dialog_ui_button">' + A.lang.UploadBtnCancel + '</span></a>' + '</div>');y.dB().append(B);A.cg.resizeFormPanel();
			this.container = B;var D = this.container.getChild([ 0, 0 ]);
			this.mx = D.getChild(1);
			this.ni = D.getChild([ 2, 0 ]);
			this.mz = D.getChild(3);
			this.np = D.getChild(4);
			this.mI = D.getChild(5);var E = this;
			this.button = this.container.getChild([ 1, 0 ]);this.button.on('click', function() {
				E.close();
			});q.ld(this);return this;
		}
		;u.prototype.mC = function(w, x, y) {
			var A = this;
			var z = A.ib.app;
			A.mI.setText(x);A.ni.getParent().hide();A.mz.hide();A.np.hide();q.mU(A, y, !!A.bT);
			delete A.bT;
			if (!x) A.close();else {
				A.button.getFirst().setHtml(z.lang.CloseBtn);A.button.addClass('cke_dialog_ui_button_ok');A.button.removeClass('cke_dialog_ui_button_cancel');
			}
			z.cg.resizeFormPanel();
		};
		u.prototype.mD = function(w, x) {
			this.ib.oW('failedUploadFile', x);this.container.addClass('ckf_FileError');this.mC('', w);
		};
		u.prototype.close = function() {
			var w = this;
			if (w.bT && w.bT.abort) {
				w.bT.abort();q.mE(w, w.file);
				w.size = 0;q.mU(w, null, !w.file);
				delete w.bT;
				delete w.file;
			}
			w.container.remove();q.lQ(w);
		};
		u.prototype.nR = function(w) {
			var x = this;
			x.speed = w / 1024;x.np.setText(CKFinder.tools.formatSpeed(x.speed, x.ib.app.lang));q.nU();
		};
		u.prototype.or = function(w) {
			...
			if (x !== 0 && x != 100)
				x = x.toFixed(1);
			y.mz.setText(y.ib.app.lang.UploadUploaded.replace('!n', x));q.or();
		};
		u.prototype.lN = function(w, x) {
			var y = this;
			y.bT = w;
			y.file = x;
			y.size = x.size;y.mx.setText(' - ' + CKFinder.tools.formatSize(x.size / 1024, y.ib.app.lang));q.nv(y);
		};
			...
			if (w) {
				...
				w.onprogress = function(A) {
					if (A.lengthComputable) {
						A.target.kC = A.loaded;y.or(A.loaded);x(A.target, y);
					}
				};
			}
			var z = new FormData();
			z.append('upload', this.file);this.bT.send(z);
			delete this.file;
		};
		function v(w, x) {
			var y = w.app,
				...
				B = x.name;
			if (!B.length) {
				w.oW('failedUploadFile');return;
			}
			var C = new u(w, B);
			if (A === 0) {
				C.mD(y.lang.Errors[202]);return;
			}
			var D = B.match(/\.([^\.]+)\s*$/),
				E = D && D[1];
			if (!E || !z.getResourceType().isExtensionAllowed(E)) {
				C.mD(w.app.lang.UploadExtIncorrect);return;
			}
			var F = parseInt(y.config.uploadMaxSize, 10);
			if (F && A > F) {
				C.mD(y.lang.Errors[203]);return;
			}
			...
			H.addEventListener('error', function(J) {
				form.removeClass('progress_visible');I('', y.lang.UploadUnknError);
			}, false);H.addEventListener('load', function(J) {
				var K = /(.*)\|(.*)/,
			...
			}, false);H.open('POST', y.connector.composeUrl('FileUpload', {
				response_type : 'txt'
			}, z.type, z));C.lN(H, x);
		}
		;
		...
		;m.add('basket', {
			...
			bz : function v(q) {
				...
				q.basketFiles = [];q.on('uiReady', function D(w) {
					var x = q.aG['foldertree.foldertree'];
					x.on('beforeAddFolder', function F(E) {
						E.removeListener();E.data.folders.push(u);
						...
						exec : function(E) {
							if (E.basketFiles.length) E.fe('', E.lang.BasketTruncateConfirm, function() {
									E.basketFiles.length = 0;E.oW('requestSelectFolder', {
										folder : E.basket
									});
								});
						}
									...
									for (var G = 0, H = F.length, I; G < H; G++)
										for (I = 0; I < E.basketFiles.length; I++) {
											if (F[G].isSameFile(E.basketFiles[I])) {
												E.basketFiles.splice(I, 1);break;
											}
									}
					...
					z.on('beforeContextMenu', function(E) {
						...
						if (!(E.data.folder instanceof a.aL.BasketFolder)) return;
						E.cancel(1);this.app.oW('requestRenderFiles', {
							files : q.basketFiles,
							mj : q.lang.BasketEmpty,
							eu : 1,
							folder : E.data.folder
						});this.app.oW('requestRepaintFolder', E.data);B(this.app);C(this.app);t();
						...
						for (var G = 0; G < F.length; G++) {
							...
							for (var J in this.app.bY._.items) {
								...
								K = q.bY.create(J);var L = K.er(q, I),
							...
							I.push('</span>');H.appendHtml(I.join(''));
						}
						this.on('beforeShowFolderFiles', function(O) {
							...
							O.removeListener();
						}, null, null, 1);this.oW('successShowFolderFiles', E.data);this.oW('afterShowFolderFiles', E.data);
					});z.on('beforeKeyboardNavigation', function H(E) {
						var F = q.aV;
						if (F && F instanceof a.aL.BasketFolder) {
							var G = E.data.db();
							if (G == 46) {
								E.cancel();q.execCommand('RemoveFileFromBasket');
							}
							if (G == 113) E.cancel();
						}
					});
	...
	a.DIALOG_RESIZE_BOTH = 3;(function() {
		...
		;a.dialog = function(P, Q) {
			...
			this.bO = T.bO;this.bO.dialog.setStyles({
				...
				V(function(aP) {
					if (aP.validate) {
						...
						if (bV === false) {
							...
							fv.data.hide = false;fv.stop();return true;
						}
					}
				});
			}, this, null, 0);this.on('cancel', function(fv) {
				V(function(aP) {
					if (aP.isChanged()) {
			...
			}, this, null, 0);this.bO.close.on('click', function(fv) {
				if (this.oW('cancel', {
						hide : true
			...
			function Y(fv) {
				...
				if (aP == 9 || aP == a.dy + 9) {
					...
					if (U._.eC) {
						...
						U.selectPage(eN);U._.tabs[eN][0].focus();
					} else W(!bV);
					X = 1;
				} else if (aP == a.eJ + 121 && !U._.eC) {
					U._.eC = true;U._.tabs[U._.gx][0].focus();
					X = 1;
				} else if ((aP == 37 || aP == 39) && U._.eC) {
					eN = aP == 37 ? q.call(U) : r.call(U);U.selectPage(eN);U._.tabs[eN][0].focus();
					X = 1;
				}
				if (X) {
					fv.stop();fv.data.preventDefault();
				}
			}
			;
			function Z(fv) {
			...
			if (f.ie6Compat) this.on('load', function(fv) {
					var aP = this.getElement(),
						bV = aP.getFirst();
					bV.remove();bV.appendTo(aP);
				}, this);
			z(this);A(this);this.bO.title.setText(R.title);
			...
			this.bO.tabs.on('click', function(fv) {
				...
				eN = aP.$.id.substr(0, aP.$.id.lastIndexOf('_'));dX.selectPage(eN);
				if (dX._.eC) {
					dX._.eC = false;
					dX._.aW = -1;W(true);
				}
		...
		function s(P, Q, R) {
			...
			this.focus = function() {
				P._.aW = this.cQ;this.element.focus();
		...
		;a.dialog.prototype = {
			...
			mn : function() {
				var P = this.hR();
				P.height = P.height - (this.bO.title.$.offsetHeight || 0) - (this.bO.footer.$.offsetHeight || 0);return P;
			},
			move : (function() {
				var P;
			...
			show : function() {
				...
				if (f.gecko && f.version < 10900) {
					...
					T.setStyle('position', 'absolute');setTimeout(function() {
						T.setStyle('position', 'fixed');
					}, 0);
				}
				this.resize(this._.hB && this._.hB.width || S.minWidth, this._.hB && this._.hB.height || S.minHeight);this.selectPage(this.dg.contents[0].id);this.reset();
				...
				if (a.dialog._.dL === null) {
					a.dialog._.dL = this;
					this._.ep = null;D(this._.app);R.on('keydown', G);R.on(f.opera ? 'keypress' : 'keyup', H);
				...
				this._.hasFocus = false;i.setTimeout(function() {
					this.layout();this.bO.dialog.setStyle('visibility', '');this.cr('load', {});this.oW('show', {});this._.app.oW('dialogShow', this);this.gh(function(W) {
						W.jW && W.jW();
					});
				}, 100, this);
			},
			...
			hide : function() {
				...
				P.remove();this.bO.dialog.setStyle('visibility', 'hidden');J(this);
				if (!this._.ep) E();else {
				...
				if (!this._.ep) {
					a.dialog._.gw = null;P.removeListener('keydown', G);P.removeListener(f.opera ? 'keypress' : 'keyup', H);
			...
			addPage : function(P) {
				...
				Z._.tabs[P.id] = [ V, U ];Z._.cU.push(P.id);Z._.gV++;
				Z._.qF = V;
				...
				U.setAttribute('name', P.id);U.appendTo(Z.bO.contents);V.unselectable();Z.bO.tabs.append(V);
				if (P.accessKey) {
			...
			selectPage : function(P) {
				...
				for (var Q in U._.tabs) {
					var R = U._.tabs[Q][0],
						S = U._.tabs[Q][1];
					if (Q != P) {
						R.removeClass('cke_dialog_tab_selected');S.hide();
					}
				}
				var T = U._.tabs[P];
				T[0].addClass('cke_dialog_tab_selected');T[1].show();
				U._.gx = P;
			...
			tQ : function(P, Q) {
				...
				if (typeof Q == 'undefined') {
					Q = S._.eO.length;S._.eO.push(new s(S, P, Q));
				} else {
		...
		function z(P) {
			...
			function W(Y) {
				...
				if (R.x + V[3] < U)
					...
					fv = aa.width - Z.width + V[1];else
					fv = R.x;
				if (R.y + V[0] < U)
					aP = -V[0];
				else if (R.y - V[2] > aa.height - Z.height - U)
					aP = aa.height - Z.height + V[2];else
					aP = R.y;
				P.move(fv, aP, 1);Y.data.preventDefault();
			}
			;
			function X(Y) {
				a.document.removeListener('mousemove', W);a.document.removeListener('mouseup', X);
				if (f.ie6Compat) {
					var Z = C.getChild(0).getFrameDocument();
					Z.removeListener('mousemove', W);Z.removeListener('mouseup', X);
				}
			}
			;P.bO.title.on('mousedown', function(Y) {
				...
				if (f.ie6Compat) {
					var Z = C.getChild(0).getFrameDocument();
					Z.on('mousemove', W);Z.on('mouseup', X);
				}
		...
		function A(P) {
			...
			function Z(bW) {
				if (P._.moved && S.lang.dir == 'rtl') {
					...
					eS.setStyle('right', bW + 'px');eS.removeStyle('left');
				} else if (!P._.moved) P.layout();
			}
			;
			function aa(bW) {
				X = P.hR();
				bW = bW.data.$;var eS = P.bO.contents,
					fv = eS.$.getElementsByTagName('iframe').length;
				if (fv) {
					Y = k.kE('<div class="cke_dialog_resize_cover" style="height: 100%; position: absolute; width: 100%;"></div>');eS.append(Y);
				}
				...
				V = a.document.getWindow().eR();a.document.on('mousemove', aT);a.document.on('mouseup', bm);
				if (f.ie6Compat) {
					var aP = C.getChild(0).getFrameDocument();
					aP.on('mousemove', aT);aP.on('mouseup', bm);
				}
			...
			;P.on('load', function() {
				...
				eS.on('mousedown', aa);P.bO.footer.append(eS, 1);
			...
			function aT(bW) {
				...
				P.resize(bV, eN);Z(am);bW.data.preventDefault();
			}
			;
			function bm() {
				a.document.removeListener('mouseup', bm);a.document.removeListener('mousemove', aT);
				if (Y) {
					Y.remove();
					Y = null;
				}
				if (f.ie6Compat) {
					...
					bW.removeListener('mouseup', bm);bW.removeListener('mousemove', aT);
				}
				if (S.lang.dir == 'rtl') {
					var eS = P._.element.getFirst(),
						fv = eS.getComputedStyle('left');
					if (fv == 'auto')
						fv = V.width - parseInt(eS.rd('right'), 10) - P.hR().width;else
						fv = parseInt(fv, 10);
					eS.removeStyle('right');
					P._.position.x += 1;P.move(fv, P._.position.y);
				}
			}
			;
		}
		...
		var B,
			...
			D = function(P) {
				...
				if (f.ie6Compat) {
					var Y = function() {
						X();arguments.callee.lw.apply(this, arguments);
					};
				...
				V.setOpacity(typeof Z != 'undefined' ? Z : 0.5);V.appendTo(a.document.bH());
			},
			E = function() {
				if (!C) return;
				var P = a.document.getWindow();
				C.remove();P.removeListener('resize', B);
			...
			G = function(P) {
				...
				U = U[U.length - 1];U.keydown && U.keydown.call(U.bf, U.dialog, U.iK);P.data.preventDefault();
			},
			H = function(P) {
				var Q = P.data.$.ctrlKey || P.data.$.metaKey,
					R = P.data.$.altKey,
				...
				U = U[U.length - 1];
				if (U.keyup) {
					U.keyup.call(U.bf, U.dialog, U.iK);P.data.preventDefault();
				}
			},
				...
				hbox : function(P, Q, R, S, T) {
					...
					var U = this._.children = Q,
						...
						Z = function() {
							...
							aa.push('</tr></tbody>');return aa.join('');
						};
				...
				vbox : function(P, Q, R, S, T) {
					...
					var U = this._.children = Q,
						...
						X = function() {
							...
							Y.push('width:' + i.cssLength(V || '100%'), ';');Y.push('"');Y.push('align="', i.htmlEncode(T && T.align || (P.eY().lang.dir == 'ltr' ? 'left' : 'right')), '" ');Y.push('><tbody>');
							...
							Y.push('</tbody></table>');return Y.join('');
						};
					n.dialog.bf.call(this, P, T || {
							type : 'vbox'
						}, S, 'div', null, null, X);
		...
		n.dialog.bf.prototype = {
			...
			setValue : function(P) {
				this.getInputElement().setValue(P);this.oW('change', {
					value : P
				});return this;
			},
			getValue : function() {
			...
			disable : function() {
				...
				P.setAttribute('disabled', 'true');P.addClass('cke_disabled');
			},
			enable : function() {
				var P = this.getInputElement();
				P.removeAttribute('disabled');P.removeClass('cke_disabled');
			},
			isEnabled : function() {
			...
			var P = {
				dQ : function(Q, R, S) {
					...
					for (var X = 0; X < T.length && (U = T[X]); X++) {
						var Y = [];
						V.push(Y);W.push(a.dialog._.gv[U.type].dQ(Q, U, Y));
					}
					return new n.dialog[R.type](Q, W, V, S, R);
				}
			};
			a.dialog.addUIElement('hbox', P);a.dialog.addUIElement('vbox', P);
		})();
		a.rB = function(P) {
			this.ry = P;
		};
					...
					return function() {
						...
						if (!S.test(U)) {
							...
							if (V && (V.select || V.focus))
								if (V.select) V.select();else V.focus();
							return false;
						}
						return true;
					};
		...
		function O(P, Q) {
			var R = function() {
				...
				T = function(U) {
					U.removeListener('ok', R);U.removeListener('cancel', S);
				};
			P.on('ok', R);P.on('cancel', S);
		}
		;i.extend(a.application.prototype, {
			openDialog : function(P, Q, R) {
				...
				if (typeof S == 'function') {
					...
					Q && Q.call(U, U);!this._.activeElement && (this._.activeElement = this.document.$.activeElement);!this._.oO && (this._.oO = this.aG['filesview.filesview'].tools.oO());U.show();return U;
				} else if (S == 'failed')
					throw new Error('[CKFINDER.dialog.openDialog] Dialog "' + P + '" failed when loading dg.');
				var V = a.document.bH(),
					W = V.$.style.cursor,
					X = this;
				V.setStyle('cursor', 'wait');a.scriptLoader.load(a.getUrl(S), function() {
					if (typeof a.dialog._.ev[P] != 'function')
						a.dialog._.ev[P] = 'failed';
					X.openDialog(P, Q);V.setStyle('cursor', W);
				}, null, null, R);return null;
			},
			hs : function(P, Q, R, S) {
				var T = this;
				setTimeout(function() {
					T.cg.openDialog('Input', function(U) {
						U.show();U.setTitle(P || T.lang.common.inputTitle);U.getContentElement('tab1', 'msg').getElement().setHtml(Q);U.getContentElement('tab1', 'input').setValue(R);O(U, function(V) {
			...
			msgDialog : function(P, Q, R) {
				...
				setTimeout(function() {
					S.cg.openDialog('Msg', function(T) {
						T.show();T.setTitle(P || S.lang.common.messageTitle);T.getContentElement('tab1', 'msg').getElement().setHtml(Q);R && O(T, function(U) {
							R();
						});
					});
				}, 0);
			...
			fe : function(P, Q, R) {
				...
				setTimeout(function() {
					S.cg.openDialog('Confirm', function(T) {
						T.show();T.setTitle(P || S.lang.common.confirmationTitle);T.getContentElement('tab1', 'msg').getElement().setHtml(Q);O(T, function(U) {
							R();
						});
					});
				}, 0);
			...
			skippedFilesDialog : function(P, Q, R, S) {
				...
				setTimeout(function() {
					T.cg.openDialog('SkippedFiles', function(U) {
						U.show();U.setTitle(P || T.lang.common.messageTitle);
						if (R) {
							U.getContentElement('tab1', 'msg').getElement().show();U.getContentElement('tab1', 'msg').getElement().setHtml(R);
						...
						U.getContentElement('tab1', 'skippedList').getElement().setHtml('<ul class="' + W + '"' + X + '>' + V + '</ul>');S && O(U, function() {
							S();
						});
					});
				}, 0);
		...
		var p = function(w) {
				...
				x.push(true);i.extend.apply(i, x);return z._;
			},
			q = {
				dQ : function(w, x, y) {
		...
		i.extend(n.dialog, {
			...
			textarea : function(w, x, y) {
				...
				var C = function() {
					...
					D.push('>', i.htmlEncode(z._['default']), '</textarea></div>');return D.join('');
				};
			...
			checkbox : function(w, x, y) {
				...
				var A = function() {
					...
					if (x['default'])
						D.checked = 'checked';
					z.checkbox = new n.dialog.bf(w, B, C, 'input', null, D);C.push(' <label for="', D.id, '">', i.htmlEncode(x.label), '</label>');return C.join('');
				};
				...
				var z = [],
					...
					B = function() {
						...
						for (var G = 0; G < x.items.length; G++) {
							...
							if (A._['default'] == J)
								M.checked = 'checked';
							v(K);v(L);z.push(new n.dialog.bf(w, K, N, 'input', null, M));N.push(' ');new n.dialog.bf(w, L, N, 'label', null, {
								'for' : M.id
							}, H[0]);C.push(N.join(''));
						}
						new n.dialog.hbox(w, [], C, D);return D.join('');
					};
			...
			select : function(w, x, y) {
				...
				var A = function() {
					...
					z.select = new n.dialog.bf(w, B, C, 'select', null, E, D.join(''));return C.join('');
				};
			...
			file : function(w, x, y) {
				...
				var A = function() {
					...
					C.push(B ? "(function(){document.open();document.domain='" + document.domain + "';" + 'document.close();' + '})()' : '0');C.push(')"></iframe>');return C.join('');
				};
				w.on('load', function() {
			...
			fileButton : function(w, x, y) {
				...
				B.onClick = function(D) {
					...
					if (!C || C.call(this, D) !== false) {
						w.getContentElement(E[0], E[1]).submit();this.disable();
					}
				};w.on('load', function() {
			...
			html : (function() {
				...
				return function(z, A, B) {
					...
					if (A.focus) {
						var H = this.focus;
						this.focus = function() {
							H.call(this);A.focus.call(this);this.oW('focus');
						};
						if (A.fM) {
							var I = this.fM;
							this.fM = I;
		...
		n.dialog.dD.prototype = i.extend(new n.dialog.bf(), {
			rW : function(w) {
				...
				if (x.iu() < 1) new h.text(w, a.document).appendTo(x);else
		...
		n.dialog.button.prototype = i.extend(new n.dialog.bf(), {
			click : function() {
				...
				w.getElement().$.blur();return false;
			},
			enable : function() {
		...
		n.dialog.ju.prototype = i.extend(new n.dialog.dD(), {
			...
			setValue : function(w) {
				w = w !== null ? w : '';return n.dialog.bf.prototype.setValue.call(this, w);
			},
			eA : true
		}, s, true);
		...
		n.dialog.checkbox.prototype = i.extend(new n.dialog.bf(), {
			...
			setValue : function(w) {
				this.getInputElement().$.checked = w;this.oW('change', {
					value : w
				});
			},
			getValue : function() {
		...
		n.dialog.file.prototype = i.extend(new n.dialog.dD(), s, {
			...
			reset : function() {
				...
				function A() {
					...
					if (y.size)
						B = y.size - (g ? 7 : 0);
					x.$.write([ '<html><head><title></title></head><body style="margin: 0; overflow: hidden; background: transparent;">', '<form enctype="multipart/form-data" method="POST" action="', i.htmlEncode(y.action), '">', '<input type="file" name="', i.htmlEncode(y.id || 'cke_upload'), '" size="', i.htmlEncode(B > 0 ? B : ''), '" />', '</form>', '</body></html>' ].join(''));x.$.close();
		...
		n.dialog.fileButton.prototype = new n.dialog.button();a.dialog.addUIElement('text', q);a.dialog.addUIElement('password', q);a.dialog.addUIElement('textarea', r);a.dialog.addUIElement('checkbox', r);a.dialog.addUIElement('radio', r);a.dialog.addUIElement('button', r);a.dialog.addUIElement('select', r);a.dialog.addUIElement('file', r);a.dialog.addUIElement('fileButton', r);a.dialog.addUIElement('html', r);i.extend(CKFinder.dialog, a.dialog);
	})();(function() {
		m.add('help', {
			bM : [ 'toolbar', 'button' ],
			bz : function q(p) {
				if (!p.config.disableHelpButton) {
					p.bD('help', {
						exec : function(r) {
							r.aG['filesview.filesview'].bn().focus();window.open(a.basePath + 'help/' + (r.lang.HelpLang || 'en') + '/index.html');
						}
		...
		function u(C, D, E, F, G) {
			...
			x();
			t = new k('div', G);t.setAttribute('id', 'ckf_gallery');t.setHtml(J.join(''));t.appendTo(G.bH());t.hide();return H;
		}
		;
		function v(C) {
			if (C && C.inPopup) {
		...
		function w(C) {
			return function() {
				C.$.activeElement && C.$.activeElement.blur();C.$.activeElement && C.$.activeElement.blur();
				if (!f.gecko) {
					C.getWindow().focus();C.bH().focus();
				}
			};
		}
		;
		...
		function z(C) {
			if (C.click) C.click();
				...
				D.initEvent('click', true, true);C.dispatchEvent(D);
			}
		}
		;
		function A(C, D, E) {
			...
			galleryCallback : function(C, D, E) {
				...
				if (!p && C.config.gallery_autoLoad) {
					...
					if (M) {
						if (G.jQuery)
							r = [ G.jQuery, G.$ ];
						L = new k('script', F);L.setAttribute('type', 'text/javascript');L.setAttribute('src', J + 'jquery.min.js');A(L, K, G);L.appendTo(I);return true;
					}
					F.appendStyleSheet(J + 'colorbox.css');
					L = new k('script', F);L.setAttribute('type', 'text/javascript');L.setAttribute('src', J + 'jquery.colorbox-min.js');A(L, K, G);L.appendTo(I);return true;
				}
				if (r.length) {
				...
				if (p) {
					...
					case 1:
						if (!H(D)) return false;
						S = u(E, D, 'ckf_gallery_' + R, H, F);s('#ckf_gallery a').colorbox(i.extend({
							minWidth : '300',
							minHeight : '200',
							maxWidth : '95%',
							maxHeight : '95%',
						...
						if (!H(D)) return false;
						S = u(E, D, 'ckf_gallery_' + R, H, F);G.jQuery('#ckf_gallery a').fancybox(i.extend({}, C.config.gallery_config, {
							onClosed : x,
							onComplete : w(F),
							afterClose : y(D),
							afterShow : w(F)
						...
						if (!H(D)) return false;
						S = u(E, D, 'prettyPhoto[ckf_gallery_' + R + ']', H, F);G.jQuery('#ckf_gallery a').prettyPhoto(i.extend({}, C.config.gallery_config, {
							callback : y(D),
						...
						if (!H(D)) return false;
						S = u(E, D, 'ckf_gallery_' + R, H, F);G.Shadowbox.qi('#ckf_gallery a', i.extend({}, C.config.gallery_config, {
							gallery : 'ckf_gallery_' + R,
							onClose : y(D),
							onFinish : w(F)
						}, true));z(t.eG('a').getItem(S).$);
						break;case 5:
						if (!H(D)) return false;
						S = u(E, D, 'lightbox[ckf_gallery_' + R + ']', H, F);z(t.eG('a').getItem(S).$);
						break;default:
						return false;
					}
					return true;
		...
		function B(C, D) {
			...
			C && C.aG['filesview.filesview'].on('afterRenderFiles', x);var E = v(D).getWindow().$;
	...
	l.gallery_autoLoad = true;(function() {
		function p(v) {
			...
			for (var y = 0; y < x.length; y++) {
				...
				if (A) {
					var B = new k(A);
					w.push([ B, B.nextSibling ]);B.remove();
				}
			}
			return w;
		}
					...
					exec : function() {
						...
						if (v.cg.inPopup && (!g && !f.opera || F.top.location.href.match(/ckfinder.html/) || F.top.name == 'CKFinderpopup')) {
							...
							if (this.bu == a.aS) {
								...
								A[3] = F.screenTop || F.screenY;F.moveTo(0, 0);
								if (!u)
							...
							if (this.bu == a.aS) {
								...
								while (H = H.getParent()) {
									I = i.getNextNumber();
									B[I] = r(H);
									H.$.og = I;H.is('html', 'body') && H.setStyle('overflow', 'hidden');H.setStyle('z-index', v.config.baseFloatZIndex - 1);
								}
								I = i.getNextNumber();
								B[I] = r(G, true);
								G.$.og = I;
								...
								K.gT().setStyles(L);!f.gecko && K.gT().setStyle('position', 'fixed');K.bH().setStyles(L);
								g ? setTimeout(function() {
									y.$.scrollTo(0, 0);
								}, 0) : y.$.scrollTo(0, 0);
								var M;
								for (var N = 0, O = J.length; N < O; N++) {
									M = J[N];M.setStyle('position', 'absolute');M.$.offsetLeft;M.setStyles({
										'z-index' : v.config.baseFloatZIndex - 1,
										left : '0px',
										top : '0px'
									});
								}
								J[0].addClass('cke_maximized');E(J);
								var P = J[0].ir();
								J[0].setStyles({
									left : -1 * P.x + 'px',
									top : -1 * P.y + 'px'
								...
								g ? setTimeout(function() {
									y.$.scrollTo(C.x, C.y);
								}, 0) : y.$.scrollTo(C.x, C.y);G.removeClass('cke_maximized');
								if (f.webkit) {
									G.setStyle('display', 'inline');setTimeout(function() {
						...
						if (Q) {
							...
							S.getChild(1).setHtml(R);S.setAttribute('title', R);S.setAttribute('href', 'javascript:void("' + R + '");');
						}
					}
				});v.bY.qW('Maximize', {
					label : w.maximize,
				...
				CKFinder.dialog.add('compressToFileName', function(t) {
						...
						contents : [ {
							...
							elements : [ {
								...
								children : [ {
									...
									validate : function() {
										if (!this.getValue()) {
											t.openMsgDialog('', t.lang.ErrorMsg.pg);return false;
										}
									}
								}, {
									type : 'html',
					...
					return {
						...
						onOk : function() {
							...
							if (x == 'manuallyrename')
								x = null;
							p.extractTo(t.getSelectedFile(), x);return true;
						},
						onCancel : function() {
							p.ma(u);return true;
						},
					...
					return {
						...
						onOk : function() {
							...
							if (x == 'manuallyrename')
								x = null;
							p.nX(t.getSelectedFolder(), p.pp.download, x);return true;
						},
						onCancel : function() {
							p.ma(u);return true;
						},
				...
				p = {
					...
					nI : function(t) {
						if (!p.filesList[p.currentItem]) {
							p.extractHere(r.getSelectedFile(), true);return;
						}
						var u = [ {
							name : 'skip'
						}, {
						...
						if (p.filesList[p.currentItem].options && q(p.filesList[p.currentItem].options, u)) {
							p.currentItem++;r.openDialog('unzipFileExists', p.nI);return;
						}
						t.show();
						var v = r.lang.ErrorMsg[p.filesList[p.currentItem].type == 'Folder' ? 'FolderExists' : 'FileExists'];
						v = '<strong>' + v.replace('%s', p.filesList[p.currentItem].name) + '</strong>';t.getContentElement('tab1', 'msg').getElement().setHtml(v);t.on('ok', function(w) {
							w.removeListener();
							...
							if (x) {
								...
								p.filesList[p.currentItem].options = y;p.currentItem++;
					...
					extract : function(t) {
						...
						if (t.extra)
							u = t.extra;
						u.fileName = t.file.name;r.connector.sendCommandPost(t.action, null, u, function(w) {
							p.filesList = [];
							if (w.getErrorNumber() == 303) {
								...
								if (z) {
									r.openDialog('unzipFileExists', p.nI);return;
								}
							...
							if (E && E.length) {
								r.openDialog('unzipDirExists', function(H) {
									...
									I = '<strong>' + I.replace('%s', E[0].getAttribute('name')) + '</strong>';H.getContentElement('tab1', 'msg').getElement().setHtml(I);
								});return;
							}
							...
							if (y && y.length) {
								...
								if (F.length) {
									r.openSkippedFilesDialog(null, F, s.extractSuccess);p.ma(r.getSelectedFolder());return true;
								}
							}
							r.openMsgDialog('OK', s.extractSuccess);p.ma(r.getSelectedFolder());return true;
						...
						if (u) {
							...
							if (!v) {
								...
								r.openSkippedFilesDialog(null, y, s.extractSuccess, function() {
									p.ma(r.getSelectedFolder());return true;
								});return;
							}
						}
						u = u || {};p.extract({
								...
								w.createNewFolder(v);p.extract({
									...
									if (!q(v, y)) p.extract({
										...
										r.openDialog('unzipDirExists', function(z) {
											...
											A = '<strong>' + A.replace('%s', v) + '</strong>';z.getContentElement('tab1', 'msg').getElement().setHtml(A);
										});return;
									}
								}, true);
						});
					...
					nX : function(t, u, v, w) {
						...
						if (v) {
							x = p.pp;
							x.fileExistsAction = v;return p.oH(t, x);
						}
						if (t.isBasket || u) {
							...
							return p.oH(t, x);
								...
								A.show();A.getContentElement('tab1', 'fileName').getElement().setValue(t.name);A.on('ok', function(B) {
									B.removeListener();
									...
									if (D) {
										...
										x.zipName = C;t.getFiles(function(E) {
											if (q(C, E)) {
												p.pp = x;r.openDialog('compressFileExists', function(F) {
													F.show();
													var G = r.lang.ErrorMsg.FileExists;
													G = '<strong>' + G.replace('%s', C) + '</strong>';F.getContentElement('tab1', 'msg').getElement().setHtml(G);
												});return false;
											} else p.oH(t, x);
										}, true);
									}
		...
		return {
			...
			bz : function(q) {
				...
				function u(x) {
					...
					y.setAttribute('id', 'cke_ui_color');y.setAttribute('type', 'text/css');return y;
				}
				;
				function v(x, y, z) {
					var A,
					...
					for (var D = 0; D < x.length; D++) {
						if (f.webkit) {
							...
							for (A = 0; A < z.length; A++) C = C.replace(z[A][0], z[A][1]);
							if (g)
								x[D].$.styleSheet.cssText = C;else x[D].setHtml(C);
						}
					}
				}
				;
				...
				i.extend(q, {
					...
					setUiColor : function(x) {
						...
						return (this.setUiColor = function(F) {
							var G = [ [ w, F ] ];
							q.uiColor = F;v([ A ], y, G);v([ B ], z, G);v(r, s, G);
						})(x);
					}
		...
		function p() {
			a.dialog.on('resize', function(q) {
				...
				v.setStyles({
					...
					y = w.getChild(7);y.setStyle('width', x.$.offsetWidth - 28 + 'px');
					y = w.getChild(4);y.setStyle('height', x.$.offsetHeight - 31 - 14 + 'px');
					y = w.getChild(5);y.setStyle('height', x.$.offsetHeight - 31 - 14 + 'px');
				}, 100);
			});
		}
		;
		...
		return {
			dQ : function(p) {
				...
				a.log('[THEME] DOM flush using document.write');p.document.$.write(A);
				function B() {
					if (f.ie6Compat)
						p.layout.oG = p.document.getWindow().eR();
				}
				;p.cr('themeLoaded');p.cr('uiReady', function() {
					B();p.cr('appReady', function() {
						B();
						if (f.ie8) {
							var C = p.document.$,
								D;
							if (C.documentMode)
								D = C.documentMode;else {
								D = 5;
								if (C.compatMode)
						...
						var G;
						function H() {
							G = G || p.document.getHead().eG('link').getItem(0);var I = 0;
							if (G) try {
							...
							if (!I) {
								window.setTimeout(H, 250);return;
							}
							if (f.ie6Compat) {
								B();p.layout.ea();setTimeout(function() {
									p.layout.ea();
								}, 500);
							} else {
								p.layout.ea(true);setTimeout(function() {
			...
			pu : function(p) {
				var q = i.getNextNumber(),
					...
					t = s.getChild(0),
					u = s.getChild(1);
				t.unselectable();u.unselectable();return {
					element : r,
					bO : {
						dialog : r.getChild(0),
						title : t,
	...
	a.application.prototype.nJ = function(p) {
		...
		r.setStyle('width', p);this.oW('resize');this.layout.ea();
	};
				...
				this.jB = i.setTimeout(function() {
					...
					if (a.bF && 1 == w && a.lS(window.top[a.nd + "\x63\x61\x74\151\x6f\x6e"][a.jG + "\163\x74"]) != a.lS(a.ed) || w == 4 || y) {
						...
						for (var C = 0; C < A.count(); C++) {
							if (A.getItem(C).rd("\x70\x6f\x73\x69\164\151\x6f\156") == "\x73\164\141\164\151\x63")
								B = 1;
						}
					...
					if (E) t(E, F.kN);
					this.jB = null;
					v = false;this.app.oW('afterRepaintLayout');
