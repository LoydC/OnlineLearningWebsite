...
function parentMenuItem(dataUrl,menuName) {
    // 获取标识数据
     var flag = true;
    ...
    // 选项卡菜单不存在
    if (flag) {
        var str = '<a href=" " class="active J_menuTab" data-id="' + dataUrl + '">' + menuName + ' <i class="fa fa-times-circle"></i></a >';
        $('.J_menuTab').removeClass('active');

        // 添加选项卡对应的iframe
        var str1 = '<iframe class="J_iframe" name="iframe9999" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
...
//    });
    ...
    if(!isNew){
		    top.$('.J_menuTab').each(function () {
		        if (top.$(this).data('title') == dataUrl) {// 选项卡已存在，激活。
